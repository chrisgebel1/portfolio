$(document).ready(function() {
    const $originPathname = window.location.pathname; // console.log($originPathname);
    const $originURL = window.location.origin; // console.log($originURL);
    let $projects = []; // tableau qui contiendra toutes les checkboxes
    // const $typePathname = $originPathname.match(/(\d+)$/)[0];
    // $typePathname = $originPathname.replace(/^[a-zA-Z:\/]+/, ''); console.log($typePathname);
    ...
    $(function () {
        $('#search-project').autocomplete({
            ...
            select: function (event, ui) {
                ...
                window.location.href = $originURL+ "/admin/project/id/" + ui.item.id;
                return false;
            }
        })
    ...
    $('#filter').click(function(){
        ...
        // console.log($param);
        window.location.href = $originURL + "/admin/project/category/" + $param;
    });
    ...
    $('.form-check').each(function () {
        ...
        $input.change(function (e){
            ...
            if ($input.is(":checked") && $input[0].getAttribute('type') === "radio") {
                window.location.href = $originURL + '/admin/project/type/' + $id;
            }
